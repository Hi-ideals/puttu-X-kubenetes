...
RUN apt-get update && apt-get install -y wget unzip && \
    wget https://github.com/grails/grails-core/releases/download/v2.4.5/grails-2.4.5.zip && \
    unzip grails-2.4.5.zip && \
    mv grails-2.4.5 /opt/grails && \
    rm grails-2.4.5.zip
ENV GRAILS_HOME=/opt/grails
ENV PATH="$GRAILS_HOME/bin:$PATH"
COPY ./Kvafsu_Puttur_Prod_2025 /app
...
RUN rm -rf ROOT*
COPY --from=build /app/target/*.war svj.war
