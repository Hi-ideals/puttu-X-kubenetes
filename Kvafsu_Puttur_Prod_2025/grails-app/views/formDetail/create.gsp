			...
			<div class="modal-content">
				...
				<div class="modal-body" style="text-align: center;">
					...
					<b
						...
						us at</b><br> <b
						style="font-size: 32px; color: green; font-family: system-ui; line-height: 26px;">recruitmentcell@kvafsu.edu.in</b>
...
<script type="text/javascript">
    $(function () {
        $("#socialcategory").change(function () {
            // Get selected value
            var selectedVal = $(this).val();

            // Allowed values that should show the RD No field
            var showList = ["SC-A", "SC-B", "SC-C", "ST", "Cat-1", "2A", "2B", "3A", "3B"];

            // Show or hide based on the selection
            if (showList.includes(selectedVal)) {
                $("#ssocialcategory").show();
            } else {
                $("#ssocialcategory").hide();
            }
        });

        // Initially hide the RD No field
        $("#ssocialcategory").hide();
    });
</script>
...
<script>
  ...
  // Countries
var appliyingPost_arr = new Array("Assistant Professor", "Associate Professor", "Professor");

// appliyingSubs
var s_a = new Array();
s_a[0] = "";
s_a[1] = "Veterinary Anatomy|Veterinary Physiology & Biochemistry (VPY-2, VBC-1)|Livestock Production and Management (LPM-1,Poultry-1)|Livestock Farm Complex (VMD-1,AGB-1,ANN-1,LPM-1)|Veterinary Clinical Complex (VMD-1,VSR-1,VGO-1,VPP-1)";
s_a[2] = "Veterinary Anatomy|Veterinary Physiology & Biochemistry (VPY/ VBC)|Livestock Production and Management|Veterinary Pathology|Veterinary Clinical Complex (VMD/VSR/VGO)";
s_a[3] = "Veterinary Anatomy|Veterinary Physiology & Biochemistry (VPY/ VBC)|Livestock Production and Management|Livestock Farm Complex (LPM-1)|Veterinary Clinical Complex (VMD/VSR/VGO)";


function populateappliyingSubs(appliyingPostElementId, appliyingSubElementId) {

    var selectedappliyingPostIndex = document.getElementById(appliyingPostElementId).selectedIndex;

    var appliyingSubElement = document.getElementById(appliyingSubElementId);

    appliyingSubElement.length = 0; // Fixed by Julian Woods
    appliyingSubElement.options[0] = new Option('Select Subject', '');
    appliyingSubElement.selectedIndex = 0;

    var appliyingSub_arr = s_a[selectedappliyingPostIndex].split("|");

    for (var i = 0; i < appliyingSub_arr.length; i++) {
        appliyingSubElement.options[appliyingSubElement.length] = new Option(appliyingSub_arr[i], appliyingSub_arr[i]);
    }
}

function populateCountries(appliyingPostElementId, appliyingSubElementId) {
    // given the id of the <select> tag as function argument, it inserts <option> tags
    var appliyingPostElement = document.getElementById(appliyingPostElementId);
    appliyingPostElement.length = 0;
    appliyingPostElement.options[0] = new Option('Select Post', '-1');
    appliyingPostElement.selectedIndex = 0;
    for (var i = 0; i < appliyingPost_arr.length; i++) {
        appliyingPostElement.options[appliyingPostElement.length] = new Option(appliyingPost_arr[i], appliyingPost_arr[i]);
    }

    // Assigned all countries. Now assign event listener for the appliyingSubs.

    if (appliyingSubElementId) {
        appliyingPostElement.onchange = function () {
            populateappliyingSubs(appliyingPostElementId, appliyingSubElementId);
        };
    }
}
...
populateCountries("appliyingPost", "appliyingSub");
   </script>
